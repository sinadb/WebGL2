...
class Mesh {
    ...
    constructor(objectData, options) {
	 ...
	 for(let i = 0; i!= this.indicesPerMaterial.length; i++){
		
		this.indices = this.indices.concat(this.indicesPerMaterial[i]);
	}
    ...
    calculateTangentsAndBitangents() {
        ...
        console.log("length",this.indicesPerMaterial.length)
        ...
        for (let i = 0; i < indices.length; i += 3) {
            ...
            const magTangent = Math.sqrt(x_tangent*x_tangent + y_tangent*y_tangent + z_tangent*z_tangent);
	    const magBitangent = Math.sqrt(x_bitangent*x_bitangent + y_bitangent*y_bitangent + z_bitangent*z_bitangent);
		// Gram-Schmidt orthogonalize
            ...
            if(!indices_set.has(i0)){
            unpacked.tangents[i0 * 3 + 0] = x_tangent/ magTangent;
            unpacked.tangents[i0 * 3 + 1] = y_tangent / magTangent;
            unpacked.tangents[i0 * 3 + 2] = z_tangent / magTangent;
	    unpacked.bitangents[i0 * 3 + 0] = x_bitangent / magBitangent;
            unpacked.bitangents[i0 * 3 + 1] = y_bitangent / magBitangent;
            unpacked.bitangents[i0 * 3 + 2] = z_bitangent / magBitangent;
	    indices_set.add(i0);
            

		}
	    if(!indices_set.has(i1)){
	    unpacked.tangents[i1 * 3 + 0] = x_tangent/ magTangent;
            unpacked.tangents[i1 * 3 + 1] = y_tangent / magTangent;
            unpacked.tangents[i1 * 3 + 2] = z_tangent / magTangent;
            unpacked.bitangents[i1 * 3 + 0] = x_bitangent / magBitangent;
            unpacked.bitangents[i1 * 3 + 1] = y_bitangent / magBitangent;
            unpacked.bitangents[i1 * 3 + 2] = z_bitangent / magBitangent;
		indices_set.add(i1);
            
		}
	    if(!indices_set.has(i2)){
	    unpacked.tangents[i2 * 3 + 0] = x_tangent/ magTangent;
            unpacked.tangents[i2 * 3 + 1] = y_tangent / magTangent;;
            unpacked.tangents[i2 * 3 + 2] = z_tangent / magTangent;
	    unpacked.bitangents[i2 * 3 + 0] = x_bitangent / magBitangent;
            unpacked.bitangents[i2 * 3 + 1] = y_bitangent / magBitangent;
            unpacked.bitangents[i2 * 3 + 2] = z_bitangent / magBitangent;
            indices_set.add(i2);
